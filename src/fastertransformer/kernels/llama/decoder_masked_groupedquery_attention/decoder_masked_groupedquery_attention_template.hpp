...
__global__ void masked_groupedquery_attention_kernel(GroupedQuery_attention_params<T> params)
{
    ...
    if (params.rotary_embedding_dim > 0 && !params.neox_rotary_style) {
        if (handle_kv) {
            ...
            apply_rotary_embedding(q, tidx, params.rotary_embedding_dim, params.rope_theta, scaled_timestep_no_pad);
        }
    }
        ...
        if (do_rotary) {
            ...
            if (handle_kv) {
                ...
                mmha::apply_rotary_embedding(
                    q, k, transpose_idx / tidx_factor, params.rotary_embedding_dim, params.rope_theta, scaled_timestep_no_pad);

                mmha::write_smem_transpose(k, k_smem, transpose_idx, smem_pitch);
            }
            else {
                mmha::apply_rotary_embedding(
                    q, transpose_idx / tidx_factor, params.rotary_embedding_dim, params.rope_theta, scaled_timestep);
            }
