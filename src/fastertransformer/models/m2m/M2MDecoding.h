...
template<typename T>
class M2MDecoding: public BaseLayer {
private:
    // meta data
...
public:
    M2MDecoding(size_t                              max_batch_size,
                 size_t                              max_seq_len,
    ...
    M2MDecoding(M2MDecoding<T> const& M2MDecoding);

    ~M2MDecoding();

    void forward(TensorMap* output_tensors, TensorMap* input_tensors, const M2MDecodingWeight<T>* Decoding_weights);
