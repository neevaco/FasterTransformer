...
template<typename T>
class M2MEncoder: public BaseLayer {
private:
    // buffer handling
...
protected:
    ...
    T*      m2m_encoder_emb_buf_    = nullptr;
    T*      m2m_encoder_in_buffer_  = nullptr;
    T*      attn_out_buf_            = nullptr;
    T*      m2m_encoder_out_buffer_ = nullptr;

    T* normed_from_tensor_  = nullptr;
    T* normed_attn_out_buf_ = nullptr;

public:
    M2MEncoder(size_t                              max_batch_size,
                size_t                              max_seq_len,
                size_t                              head_num,
    ...
    M2MEncoder(M2MEncoder<T> const& m2m_layer);

    ~M2MEncoder();

    void forward(TensorMap* output_tensors, TensorMap* input_tensors, const M2MEncoderWeight<T>* m2m_encoder_weights);
