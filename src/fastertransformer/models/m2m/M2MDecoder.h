...
template<typename T>
class M2MDecoder: public BaseLayer {
private:
    // buffer handling
...
public:
    M2MDecoder(size_t                              max_batch_size,
                size_t                              head_num,
    ...
    M2MDecoder(M2MDecoder<T> const& decoder);

    ~M2MDecoder();

    void forward(std::vector<Tensor>*                           output_tensors,
                 const std::vector<Tensor>*                     input_tensors,
                 const std::vector<M2MDecoderLayerWeight<T>*>* decoder_layer_weights);
