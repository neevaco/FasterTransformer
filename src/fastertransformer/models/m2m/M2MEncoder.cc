...
#include "src/fastertransformer/models/m2m/M2MEncoder.h"
...
template<typename T>
M2MEncoder<T>::M2MEncoder(size_t                              max_batch_size,
                            size_t                              max_seq_len,
                            size_t                              head_num,
...
template<typename T>
M2MEncoder<T>::M2MEncoder(M2MEncoder<T> const& m2m_encoder):
    BaseLayer(m2m_encoder),
    max_batch_size_(m2m_encoder.max_batch_size_),
    max_seq_len_(m2m_encoder.max_seq_len_),
    head_num_(m2m_encoder.head_num_),
    size_per_head_(m2m_encoder.size_per_head_),
    inter_size_(m2m_encoder.inter_size_),
    d_model_(m2m_encoder.d_model_),
    hidden_units_(m2m_encoder.hidden_units_),
    num_layer_(m2m_encoder.num_layer_),
    num_bucket_or_max_seq_len_(m2m_encoder.num_bucket_or_max_seq_len_),
    max_distance_(m2m_encoder.max_distance_),
    sm_(m2m_encoder.sm_),
    q_scaling_(m2m_encoder.q_scaling_),
    ...
    enable_custom_all_reduce_(m2m_encoder.enable_custom_all_reduce_)
{
    initialize();
}

template<typename T>
M2MEncoder<T>::~M2MEncoder()
{
...
template<typename T>
void M2MEncoder<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
void M2MEncoder<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        ...
        m2m_encoder_emb_buf_ = (T*)allocator_->reMalloc(
            m2m_encoder_emb_buf_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
        m2m_encoder_in_buffer_ = (T*)allocator_->reMalloc(
            m2m_encoder_in_buffer_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
        attn_out_buf_ =
...
template<typename T>
void M2MEncoder<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    h_pinned_token_num_ptr_ = (size_t*)allocator_->reMalloc(h_pinned_token_num_ptr_, sizeof(size_t), true, true);
    ...
    m2m_encoder_emb_buf_ =
        (T*)allocator_->reMalloc(m2m_encoder_emb_buf_, sizeof(T) * batch_size * seq_len * d_model_, false);
    m2m_encoder_in_buffer_ =
        (T*)allocator_->reMalloc(m2m_encoder_in_buffer_, sizeof(T) * batch_size * seq_len * d_model_, false);
...
void M2MEncoder<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        allocator_->free((void**)(&m2m_encoder_emb_buf_));
        allocator_->free((void**)(&m2m_encoder_in_buffer_));
...
template<typename T>
bool M2MEncoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool M2MEncoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool M2MEncoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int M2MEncoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
void M2MEncoder<T>::forward(TensorMap*                  output_tensors,
                             TensorMap*                  input_tensors,
                             const M2MEncoderWeight<T>* m2m_encoder_weights)
{
    // input_tensors:
...
//         buf = new T[st];
//         cudaMemcpy(buf, m2m_encoder_emb_buf_, sizeof(T) * st, cudaMemcpyDeviceToHost);
//         printf("m2m_encoder_emb_buf_\n");
...
// }
        ...
        switch (attention_type_) {
            ...
            case AttentionType::UNFUSED_PADDED_MHA: {
                ...
                if (use_inputs_embeds_buffer) {
                    cudaMemcpyAsync(m2m_encoder_emb_buf_,
                                    input_tensors->at("inputs_embeds").getPtrWithOffset<T>(d_model_offset),
                ...
                sync_check_cuda_error();
                h_token_num               = local_batch_size * request_seq_len;
                m2m_encoder_input_ptr    = m2m_encoder_emb_buf_;
                m2m_encoder_output_ptr   = output_tensors->at("output_hidden_state").getPtr<T>() + d_model_offset;
            ...
            case AttentionType::FUSED_MHA: {
                ...
                // if (pipeline_para_.rank_ == 0) {
                //     invokeRemovePadding(
                //         m2m_encoder_in_buffer_, m2m_encoder_emb_buf_, padding_offset_, h_token_num, d_model_,
                //         stream_);
                //     sync_check_cuda_error();
                // }
                // sync_check_cuda_error();
            ...
            case AttentionType::FUSED_PADDED_MHA: {
                ...
                // m2m_encoder_input_ptr = m2m_encoder_emb_buf_ + d_model_offset;
                // m2m_encoder_output_ptr = output_tensors->at("output_hidden_state").getPtr<T>() + d_model_offset;
                // break;
            }
            default: {
        ...
        for (uint i = 0; i < num_layer_; i++) {
            ...
            T* from_tensor = (i == 0 ? m2m_encoder_input_ptr : m2m_encoder_output_ptr);
            T* out_tensor  = m2m_encoder_output_ptr;

            M2MEncoderLayerWeight<T>* layer_weight = m2m_encoder_weights->m2m_encoder_layer_weights[i];
...

            {
                ...
                attn_input_tensors.insertIfValid(
                    ...
                    Tensor{MEMORY_GPU,
                           ...
                           m2m_encoder_weights->position_embedding_type == PositionEmbeddingType::relative ?
                               relative_attention_bias_ :
                               nullptr});
...
template class M2MEncoder<float>;
template class M2MEncoder<half>;
#ifdef ENABLE_BF16
template class M2MEncoder<__nv_bfloat16>;
#endif
