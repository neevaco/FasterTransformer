...
#include "src/fastertransformer/models/m2m/M2MDecoder.h"

namespace fastertransformer {

template<typename T>
void M2MDecoder<T>::initialize()
{
...
template<typename T>
void M2MDecoder<T>::allocateBuffer()
{
...
template<typename T>
void M2MDecoder<T>::allocateBuffer(size_t batch_size)
{
...
template<typename T>
bool M2MDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
M2MDecoder<T>::M2MDecoder(size_t                              max_batch_size,
                            size_t                              head_num,
...
template<typename T>
M2MDecoder<T>::M2MDecoder(M2MDecoder<T> const& decoder):
...
template<typename T>
void M2MDecoder<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
bool M2MDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool M2MDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool M2MDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int M2MDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
void M2MDecoder<T>::forward(std::vector<Tensor>*                           output_tensors,
                             const std::vector<Tensor>*                     input_tensors,
                             const std::vector<M2MDecoderLayerWeight<T>*>* decoder_layer_weight)
{
    // input tensors:
...
template class M2MDecoder<float>;
template class M2MDecoder<half>;
#ifdef ENABLE_BF16
template class M2MDecoder<__nv_bfloat16>;
#endif
