...
#include "src/fastertransformer/models/m2m/M2MDecoding.h"
...
template<typename T>
void M2MDecoding<T>::initialize()
{
    decoder_ = new M2MDecoder<T>(0,  // max_batch_size_ * beam_width_,
                                  head_num_,
                                  size_per_head_,
                                  inter_size_,
                                  d_model_,
...
template<typename T>
void M2MDecoding<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
template<typename T>
void M2MDecoding<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
M2MDecoding<T>::M2MDecoding(size_t                              max_batch_size,
                              size_t                              max_seq_len,
...
template<typename T>
M2MDecoding<T>::M2MDecoding(M2MDecoding<T> const& decoding):
    BaseLayer(decoding),
...
template<typename T>
void M2MDecoding<T>::registerCallback(callback_sig* fn, void* ctx)
{
}

template<typename T>
void M2MDecoding<T>::unRegisterCallback()
{
}


template<typename T>
void M2MDecoding<T>::forward(TensorMap*                   output_tensors,
                              TensorMap*                   input_tensors,
                              const M2MDecodingWeight<T>* decoding_weights)
{
    // input_tensors:
...
template class M2MDecoding<float>;
template class M2MDecoding<half>;
#ifdef ENABLE_BF16
template class M2MDecoding<__nv_bfloat16>;
#endif
