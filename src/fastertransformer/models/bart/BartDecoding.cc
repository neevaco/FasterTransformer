...
template<typename T>
void BartDecoding<T>::initialize()
{
    decoder_ = new BartDecoder<T>(0,  // max_batch_size_ * beam_width_,
                                  head_num_,
                                  size_per_head_,
                                  inter_size_,
                                  d_model_,
...
template<typename T>
void BartDecoding<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
template<typename T>
void BartDecoding<T>::freeBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void BartDecoding<T>::setStream(cudaStream_t stream)
{
...
// }
                ...
                }
    {
        ...
        printf("output_ids_buf_ before forward: %d at step: %d\n", batch_size, step);
        for (int i=0; i<st; i++) {
            printf("%d ", buf[i]);
        }
        printf("\n");
                ...
                for (const &string key : dynamic_decode_output_tensors.keys()) {
                    printf("%s ", key.c_str());
                }
                printf("\n");
            }
...
template class BartDecoding<float>;
template class BartDecoding<half>;
#ifdef ENABLE_BF16
template class BartDecoding<__nv_bfloat16>;
#endif
