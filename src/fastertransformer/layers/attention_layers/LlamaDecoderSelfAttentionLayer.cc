...
template<typename T>
void LlamaDecoderSelfAttentionLayer<T>::allocateBuffer()
{
...
void LlamaDecoderSelfAttentionLayer<T>::allocateBuffer(size_t batch_size)
{
    ...
    if (local_kv_head_num_ != local_head_num_) {
        qkv_buf_tmp_ =
            reinterpret_cast<T*>(allocator_->reMalloc(qkv_buf_tmp_, type_size * batch_size * size_per_head_ * local_q_kv_head_num, false));
    } else {
...
template<typename T>
bool LlamaDecoderSelfAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
