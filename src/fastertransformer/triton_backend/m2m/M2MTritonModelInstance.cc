...
{
    auto* const model  = reinterpret_cast<M2MTritonModelInstance<T>*>(ctx);
    auto const  result = M2MTritonModelInstance<T>::convert_outputs(*output_tensors);

    model->stream_cb_(result, model->stream_ctx_);
}

template<typename T>
M2MTritonModelInstance<T>::M2MTritonModelInstance(std::unique_ptr<ft::M2MEncoder<T>>        m2m_encoder,
                                                std::unique_ptr<ft::M2MDecoding<T>>       m2m_decoding,
                                                std::shared_ptr<ft::M2MEncoderWeight<T>>  m2m_encoder_weight,
                                                std::shared_ptr<ft::M2MDecodingWeight<T>> m2m_decoding_weight,
                                                std::unique_ptr<ft::Allocator<ft::AllocatorType::CUDA>> allocator,
                                                std::unique_ptr<ft::cublasAlgoMap>                      cublas_algo_map,
                                                std::unique_ptr<std::mutex>          cublas_wrapper_mutex,
                                                std::unique_ptr<ft::cublasMMWrapper> cublas_wrapper,
                                                std::unique_ptr<cudaDeviceProp>      cuda_device_prop_ptr):
    m2m_encoder_(std::move(m2m_encoder)),
    m2m_decoding_(std::move(m2m_decoding)),
    m2m_encoder_weight_(m2m_encoder_weight),
    m2m_decoding_weight_(m2m_decoding_weight),
...
template<typename T>
ft::TensorMap
M2MTritonModelInstance<T>::convert_inputs(std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
...
M2MTritonModelInstance<T>::convert_outputs(ft::TensorMap& output_tensors)
{
...
M2MTritonModelInstance<T>::forward(std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    ...
    ft::TensorMap encoder_output_tensors(
        {{"output_hidden_state",
          ft::Tensor{ft::MEMORY_GPU,
                     ft::getTensorType<T>(),
                     std::vector<size_t>{request_batch_size, mem_max_seq_len, m2m_encoder_->getDModel()},
                     d_encoder_outputs_}}});
    ...
    try {
        if (stream_cb_ != nullptr) {
            m2m_decoding_->registerCallback(triton_stream_callback<T>, this);
        }
...
//             for (int j=0; j<m2m_encoder_->getDModel(); j++) {
//                 if (j<10 || j >m2m_encoder_->getDModel()-10) {
//                     printf("%f ", double(buf[i*m2m_encoder_->getDModel()+j]));
//                 }
//             }
//             printf("\n");
...
// }
        ...
        m2m_decoding_->forward(&decoding_output_tensors, &decoding_input_tensors, m2m_decoding_weight_.get());
        if (stream_cb_ != nullptr) {
            m2m_decoding_->unRegisterCallback();
        }
    }
    catch (...) {
...
template<typename T>
M2MTritonModelInstance<T>::~M2MTritonModelInstance()
{
    freeBuffer();
}

template<typename T>
void M2MTritonModelInstance<T>::allocateBuffer(const size_t request_batch_size,
                                              const size_t beam_width,
                                              ...
                                              const size_t mem_max_seq_len)
{
    ...
    d_encoder_outputs_ = (T*)(allocator_->reMalloc(
        d_encoder_outputs_, sizeof(T) * request_batch_size * mem_max_seq_len * m2m_encoder_->getDModel(), false));
    d_sequence_lengths_ =
...
template<typename T>
void M2MTritonModelInstance<T>::freeBuffer()
{
...
template struct M2MTritonModelInstance<float>;
template struct M2MTritonModelInstance<half>;
#ifdef ENABLE_BF16
template struct M2MTritonModelInstance<__nv_bfloat16>;
#endif
