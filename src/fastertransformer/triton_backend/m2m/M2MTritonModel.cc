...
#include "src/fastertransformer/triton_backend/m2m/M2MTritonModelInstance.h"
#include "src/fastertransformer/triton_backend/transformer_triton_backend.hpp"
#include "src/fastertransformer/utils/allocator.h"

namespace ft = fastertransformer;

std::shared_ptr<AbstractTransformerModel> AbstractTransformerModel::createM2MModel(std::string model_dir)
{
    printf("createM2MModel\n");
    ...
    if (data_type == "fp16") {
        // return std::make_shared<M2MTritonModel<half>>(reader, model_dir);
        return std::make_shared<M2MTritonModel<half>>(1, 1, 0, model_dir, 0);
    }
#ifdef ENABLE_BF16
    else if (data_type == "bf16") {
        return std::make_shared<M2MTritonModel<__nv_bfloat16>>(1, 1, 0, model_dir, 0);
    }
#endif
    else if (data_type == "fp32") {
        return std::make_shared<M2MTritonModel<float>>(1, 1, 0, model_dir, 0);
    }
    else {
...
template<typename T>
M2MTritonModel<T>::M2MTritonModel(INIReader reader, std::string model_dir): model_dir_(model_dir)
{
    // encoder
...
template<typename T>
M2MTritonModel<T>::M2MTritonModel(size_t      tensor_para_size,
    ...
    encoder_shared_weights_(std::vector<std::shared_ptr<ft::M2MEncoderWeight<T>>>(ft::getDeviceCount())),
    decoding_shared_weights_(std::vector<std::shared_ptr<ft::M2MDecodingWeight<T>>>(ft::getDeviceCount())),
...
M2MTritonModel<T>::createModelInstance(int                                                               device_id,
                                      int                                                               rank,
                                      cudaStream_t                                                      stream,
                                      ...
                                      std::shared_ptr<ft::AbstractCustomComm> custom_all_reduce_comm)
{
    ...
    // NOTE: fmha doesn't support m2m-style relative position bias
    ...
    auto encoder = std::make_unique<ft::M2MEncoder<T>>(ft::M2MEncoder<T>(0,
                                                                       0,
                                                                       encoder_head_num_,
    ...
    auto decoding = std::make_unique<ft::M2MDecoding<T>>(ft::M2MDecoding<T>(0,
                                                                          0,
                                                                          0,
                                                                          0,
                                                                          decoding_head_num_,
    ...
    return std::unique_ptr<M2MTritonModelInstance<T>>(new M2MTritonModelInstance<T>(std::move(encoder),
                                                                                  std::move(decoding),
...
template<typename T>
void M2MTritonModel<T>::createSharedWeights(int device_id, int rank)
{   
    ft::check_cuda_error(cudaSetDevice(device_id));
    ...
    encoder_shared_weights_[device_id] =
        std::make_shared<ft::M2MEncoderWeight<T>>(encoder_head_num_,
                                                 encoder_size_per_head_,
                                                 encoder_d_model_,
                                                 encoder_inter_size_,
                                                 encoder_vocab_size_,
    ...
    decoding_shared_weights_[device_id] =
        std::make_shared<ft::M2MDecodingWeight<T>>(decoding_head_num_,
                                                  decoding_size_per_head_,
                                                  decoding_d_model_,
                                                  decoding_inter_size_,
                                                  decoding_vocab_size_,
...
template<typename T>
std::string M2MTritonModel<T>::toString()
{
    std::stringstream ss;
...
template<typename T>
void M2MTritonModel<T>::createCustomComms(std::vector<std::shared_ptr<ft::AbstractCustomComm>>* custom_all_reduce_comms,
                                         int                                                   world_size)
{
...
template<typename T>
int M2MTritonModel<T>::getTensorParaSize()
{
    return tensor_para_size_;
}

template<typename T>
int M2MTritonModel<T>::getPipelineParaSize()
{
    return pipeline_para_size_;
}

template struct M2MTritonModel<float>;
template struct M2MTritonModel<half>;
#ifdef ENABLE_BF16
template struct M2MTritonModel<__nv_bfloat16>;
#endif
