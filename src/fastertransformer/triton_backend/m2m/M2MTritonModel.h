...
template<typename T>
struct M2MTritonModel: public AbstractTransformerModel {
    M2MTritonModel(INIReader reader, std::string model_dir);

    M2MTritonModel(size_t      tensor_para_size,
                  size_t      pipeline_para_size,
                  int         enable_custom_all_reduce,
                  std::string model_dir,
                  int         int8_mode);

    ~M2MTritonModel() = default;
...
private:
    ...
    std::vector<std::shared_ptr<ft::M2MEncoderWeight<T>>>  encoder_shared_weights_;
